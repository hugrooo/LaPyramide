...
class DistributionScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      children: [
          ...
          children: activePlayer.hand
              ...
              .map((e) => SizedBox(
                      ...
                      child: PlayingCardWidget(
                          card: e.value, faceUp: e.value.isFaceUp))
                  .animate()
        ...
        if (state.pendingDrinks.isNotEmpty &&
            state.pendingDrinks.first.toPlayerId.isEmpty)
          if (isMyTurn)
            ...
            Center(
              child: Text(
                ...
                style:
                    const TextStyle(color: PyraTheme.primaryPink, fontSize: 18),
              ),
            )
        else if (isMyTurn)
          _buildChoices(context, ref, cardIndex)
              .animate()
            ...
            child: Column(
              children: [
                ...
                Text(
                  ...
                  style: const TextStyle(
                      color: PyraTheme.textSecondary, fontSize: 18),
                ),
              ],
            ),
  ...
  Widget _buildMyHand(GameState state, String currentUserId) {
    ...
    return GlassContainer(
      ...
      child: Column(
        children: [
          const Text('Tes cartes (mémorise-les !) :',
  ...
  Widget _buildPlayerSelection(
      BuildContext context, WidgetRef ref, Player activePlayer) {
    ...
    return Column(
      children: [
        ...
        Wrap(
          ...
          children:
              state.players.where((p) => p.id != activePlayer.id).map((p) {
            return GestureDetector(
              ...
              child: GlassContainer(
                ...
                opacity: 0.3,
                border:
                    Border.all(color: PyraTheme.primaryPink.withOpacity(0.5)),
                child: Row(
  ...
  String _getQuestion(int cardIndex) {
    switch (cardIndex) {
      ...
      case 1:
        return 'Plus grand, Plus petit ou Égal ?';
      case 2:
        return 'Intérieur ou Extérieur ?';
      case 3:
        return 'Quel signe ?';
      default:
        return '?';
  ...
  Widget _buildChoices(BuildContext context, WidgetRef ref, int cardIndex) {
    ...
    void onChoice(dynamic choice) {
      final newState =
          DistributionLogic.processChoice(state: state, choice: choice);
        ...
        return Row(
          ...
          children: [
            _ChoiceButton(
                label: '🔴 Rouge',
                color: Colors.red,
                onPressed: () => onChoice('rouge')),
            const SizedBox(width: 16),
            _ChoiceButton(
                label: '⚫ Noir',
                color: Colors.black,
                onPressed: () => onChoice('noir')),
          ],
        );
      case 1:
        return Wrap(
          ...
          children: [
            _ChoiceButton(
                ...
                onPressed: () => onChoice('plus')),
            _ChoiceButton(
                label: '⏸️ Égal',
                color: Colors.grey,
                onPressed: () => onChoice('egal')),
            _ChoiceButton(
                label: '⬇️ Moins',
                color: PyraTheme.primaryPurple,
                onPressed: () => onChoice('moins')),
          ],
        );
      case 2:
        return Row(
          ...
          children: [
            _ChoiceButton(
                label: '↔️ Intérieur',
                color: PyraTheme.primaryPink,
                onPressed: () => onChoice('interieur')),
            const SizedBox(width: 16),
            _ChoiceButton(
                label: '↕️ Extérieur',
                color: PyraTheme.primaryPurple,
                onPressed: () => onChoice('exterieur')),
          ],
        );
      case 3:
        return Wrap(
          ...
          children: [
            _ChoiceButton(
                label: '♠️ Pique',
                color: Colors.black,
                onPressed: () => onChoice(CardSuit.spades)),
            _ChoiceButton(
                label: '♥️ Cœur',
                color: Colors.red,
                onPressed: () => onChoice(CardSuit.hearts)),
            _ChoiceButton(
                label: '♦️ Carreau',
                color: Colors.red,
                onPressed: () => onChoice(CardSuit.diamonds)),
            _ChoiceButton(
                label: '♣️ Trèfle',
                color: Colors.black,
                onPressed: () => onChoice(CardSuit.clubs)),
          ],
        );
      default:
